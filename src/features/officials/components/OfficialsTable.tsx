...
export function OfficialsTable() {
  ...
  const columns: ColumnDef<Official>[] = [
    ...
    },
    {
      ...
      header: 'Status',
      cell: ({ row }) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${row.getValue('status') === OfficialStatus.ACTIVE ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
          {row.getValue('status')}
        </span>
      ),
    },
